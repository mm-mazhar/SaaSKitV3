// tests/orpc/routers/workspace.test.ts
// Feature: orpc-integration, Properties 11, 16: Workspace Router Tests
 ...
 * This is a copy of the function from workspace.ts to avoid importing the router
...
/**
 * Name validation logic - mirrors the validation in workspace router
 */
...
describe('Workspace Router Properties', () => {
  /**
   * Property 11: Entity Name Validation (Workspace portion)
   * For any workspace name that is empty or exceeds 20 characters,
   * the create procedure SHALL reject the input with a validation error.
   * **Validates: Requirements 4.1**
   */
  describe('Property 11: Entity Name Validation (Workspace)', () => {
  ...
  /**
   * Property 16: Workspace Slug Uniqueness
   * For any workspace creation with a given name and user, the generated slug
   * SHALL contain a name-based prefix, user ID prefix, and timestamp to ensure uniqueness.
   * **Validates: Requirements 4.3**
   */
  describe('Property 16: Workspace Slug Uniqueness', () => {
    it('generates slug with name-based prefix', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.uuid(),
          async (name, userId) => {
            const slug = generateWorkspaceSlug(name, userId)
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (name, userId1, userId2) => {
            ...
            const slug1 = generateWorkspaceSlug(name, userId1)
            const slug2 = generateWorkspaceSlug(name, userId2)
