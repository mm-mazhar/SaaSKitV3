...
import { ROLES, LIMITS, CREDITS_FREE } from '@/lib/constants'
...
describe('Onboarding & Multi-Tenancy Structure', () => {
  ...
  describe('Test 1.1: New User Default State', () => {
    it('should create a new user with a "Personal" organization and default credits', async () => {
      ...
      expect(organization.credits).toBe(CREDITS_FREE)
    ...
    itIf(MULTI_ORG_ENABLED)('should prevent creating multiple primary organizations for the same user', async () => {
      const testUser = await TestUtils.createTestUser()
      testUserId = testUser.id
      ...
      expect(firstOrg.credits).toBe(CREDITS_FREE)
  ...
  describe('Test 1.2: Data Isolation', () => {
    itIf(MULTI_ORG_ENABLED)('should isolate workspaces between different organizations', async () => {
      const testUser = await TestUtils.createTestUser()
      testUserId = testUser.id
      ...
      // Create a workspace in Work Corp
      ...
      // Verify: Workspace exists in Work Corp
      const workOrgWorkspaces = await DatabaseHelpers.getWorkspacesByOrganization(workOrg.id)
      expect(workOrgWorkspaces).toHaveLength(1)
      expect(workOrgWorkspaces[0].name).toBe('Workspace Alpha')

      // Verify: Workspace does NOT exist in Default Organization
      const defaultOrgWorkspaces = await DatabaseHelpers.getWorkspacesByOrganization(defaultOrg.id)
      expect(defaultOrgWorkspaces).toHaveLength(0)

      // Verify: Workspaces are properly isolated by organization
      const allWorkspaces = await testDb.workspace.findMany({
        where: {
          organization: {
            members: {
      ...
      expect(allWorkspaces[0].organizationId).toBe(workOrg.id)
      expect(allWorkspaces[0].organizationId).not.toBe(defaultOrg.id)
    })
    ...
    itIf(MULTI_ORG_ENABLED)('should maintain proper organization context when switching between organizations', async () => {
      const testUser = await TestUtils.createTestUser()
      testUserId = testUser.id
      ...
      // Verify: When in Personal context, only see Personal workspaces
      const personalContext = await DatabaseHelpers.getWorkspacesByOrganization(personalOrg.id)
      expect(personalContext).toHaveLength(1)
      expect(personalContext[0].name).toBe('Personal Workspace')

      // Verify: When in Business context, only see Business workspaces
      const businessContext = await DatabaseHelpers.getWorkspacesByOrganization(businessOrg.id)
      expect(businessContext).toHaveLength(1)
      expect(businessContext[0].name).toBe('Business Workspace')

      // Verify: Workspaces don't leak between contexts
  ...
  describe('Additional Multi-Tenancy Tests', () => {
    it('should enforce organization limits per user', async () => {
      ...
      const organizations = []
      for (let i = 0; i < LIMITS.MAX_ORGANIZATIONS_PER_USER; i++) {
      ...
      expect(organizations).toHaveLength(LIMITS.MAX_ORGANIZATIONS_PER_USER)

      // Verify: Attempting to create one more organization should fail
      await expect(
