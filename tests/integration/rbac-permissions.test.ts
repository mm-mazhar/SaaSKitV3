...
describe('RBAC (Roles & Permissions)', () => {
  ...
  describe('Test 2.1: Member Restrictions', () => {
    ...
    it('should allow MEMBER to create workspaces', async () => {
      await setupOrgWithAllRoles()

      const canCreate = can(ROLES.MEMBER, 'workspace:create')
      expect(canCreate).toBe(true)
    })

    it('should allow MEMBER to update workspaces', async () => {
      await setupOrgWithAllRoles()

      const canUpdate = can(ROLES.MEMBER, 'workspace:update')
      expect(canUpdate).toBe(true)
    })

    it('should deny MEMBER permission to delete workspaces', async () => {
      await setupOrgWithAllRoles()

      const canDelete = can(ROLES.MEMBER, 'workspace:delete')
      expect(canDelete).toBe(false)
    })
  })
  ...
  describe('Test 2.2: Admin Restrictions', () => {
    ...
    it('should allow ADMIN full workspace permissions', async () => {
      await setupOrgWithAllRoles()

      expect(can(ROLES.ADMIN, 'workspace:create')).toBe(true)
      expect(can(ROLES.ADMIN, 'workspace:update')).toBe(true)
      expect(can(ROLES.ADMIN, 'workspace:delete')).toBe(true)
    })
  })
  ...
  describe('Test 2.3: Owner Permissions', () => {
    ...
    it('should allow OWNER all workspace permissions', async () => {
      await setupOrgWithAllRoles()

      expect(can(ROLES.OWNER, 'workspace:create')).toBe(true)
