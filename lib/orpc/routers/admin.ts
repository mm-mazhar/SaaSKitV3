...
export const adminRouter = {
  ...
  getDashboardStats: superAdminProcedure
    ...
    .handler(async ({ context }) => {
      ...
      const [totalUsers, totalOrgs, proSubCount, dealerCoreSubCount, dealerPlusSubCount] = await Promise.all([
        db.user.count(),
        ...
        dealerCorePlan
          ? db.subscription.count({ 
              where: { status: 'active', planId: dealerCorePlan.stripePriceId }
            })
          : Promise.resolve(0),
        dealerPlusPlan
          ? db.subscription.count({
              where: { status: 'active', planId: dealerPlusPlan.stripePriceId }
            })
          : Promise.resolve(0),
      ])
      ...
      const dealerCoreRevenue = dealerCoreSubCount * Number(dealerCorePlan?.price || 0)
      const dealerPlusRevenue = dealerPlusSubCount * Number(dealerPlusPlan?.price || 0)
   ...
   * Returns orgs with member/workspace counts and subscription status
   */
   ...
   * Returns org with members, workspaces, subscription, and invoice history
   */
  getOrganizationDetails: superAdminProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      return {
        ...
        workspaces: org.workspaces.map(p => ({
          id: p.id,
          name: p.name,
          slug: p.slug,
   ...
   * Returns recent users, orgs, subscriptions, and workspaces
   */
  getRecentActivity: superAdminProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      const [newUsers, newOrgs, newSubs, newWorkspaces] = await Promise.all([
        db.user.findMany({ 
          take: 4, 
          orderBy: { createdAt: 'desc' }, 
      ...
      type ActivityItem = {
        id: string
        type: 'user_join' | 'org_create' | 'sub_new' | 'workspace_create'
        title: string
        description: string
        createdAt: Date
      }
      ...
      newWorkspaces.forEach(p => activities.push({
        id: `p-${p.id}`,
