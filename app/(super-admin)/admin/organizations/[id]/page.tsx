...
export default async function OrgDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  const org = await prisma.organization.findUnique({
    ...
    include: {
      ...
      workspaces: { orderBy: { createdAt: 'desc' }, take: 10 },
  ...
  return (
    <div className="flex flex-col p-4 md:p-8 gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Total Workspaces</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{org.workspaces.length}</div></CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="members" className="w-full">
        ...
        <TabsContent value="workspaces" className="mt-4">
          <Card>
            <CardHeader><CardTitle>Workspaces</CardTitle><CardDescription>Recent workspaces created within this organization.</CardDescription></CardHeader>
            <CardContent>
               <Table>
                <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Slug</TableHead><TableHead>Created</TableHead></TableRow></TableHeader>
                <TableBody>
                  {org.workspaces.map((p) => (
                    <TableRow key={p.id}>
