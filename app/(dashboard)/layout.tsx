...
async function DashboardGroupLayout({ children }: { children: ReactNode }) {
  ...
  let organizations = await rpc.org.list() as { id: string; name: string; slug: string; members: { role: string }[]; isPrimary?: boolean }[]
  ...
  if (organizations.length === 0 && userRow?.createdAt) {
    const defaultOrgName = generateOrgNameFromEmail(user.email || 'user@example.com')
    try {
      ...
      await WorkspaceService.createWorkspace(
        ...
        `${slugify(defaultWorkspaceName)}-${user.id.substring(0, 8)}-${Date.now()}`
      )
  ...
  const { getActiveWorkspaceId } = await import('@/lib/workspace-helper')

  let workspaces: WorkspaceListOutput = []
  ...
  if (effectiveOrgId) {
    try {
      ...
      workspaces = await WorkspaceService.getOrganizationWorkspaces(user.id, effectiveOrgId)
    } catch (e) {
      console.error('[Dashboard Layout] Failed to fetch workspaces:', e)
    }
  }
    ...
    <QueryProvider>
      <ToastProvider>
        ...
        <main className="flex-1 flex flex-col overflow-y-auto">
          ...
          />
          <div className="px-4 md:px-8 pt-2 md:pt-4 pb-4 md:pb-8 flex-1">
            {children}
          </div>
