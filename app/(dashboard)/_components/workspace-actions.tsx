// app/(dashboard)/_components/workspace-actions.tsx
...
export function WorkspaceActions({ workspaceId, defaultName, userRole }: { workspaceId: string; defaultName: string; userRole?: string }) {
  ...
  const updateMutation = useORPCMutation(() =>
    orpc.workspace.updateName.mutationOptions({
      onSuccess: () => {
        show({ title: 'Saved', description: 'Workspace renamed successfully', variant: 'success' })
        setOpenRename(false)
        router.refresh()
      },
      onError: (err: Error) => {
  ...
  const deleteMutation = useORPCMutation(() =>
    orpc.workspace.delete.mutationOptions({
      onSuccess: () => {
        show({ title: 'Deleted', description: 'Workspace deleted successfully', variant: 'success' })
        setOpenDelete(false)
        router.refresh()
      },
      onError: (err: Error) => {
  ...
  return (
    <div className='relative'>
      {userRole === 'MEMBER' ? (
        <div
          aria-label='Workspace actions disabled'
          className='text-muted-foreground/60 opacity-50 cursor-not-allowed flex items-center justify-center rounded-md p-1'
        >
